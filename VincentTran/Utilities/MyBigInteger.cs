...
namespace VincentTran.Utilities
{
	public class MyBigInteger : ICloneable, IComparable<MyBigInteger>
	{
		...
		public static int MaxDigits = 110997;
		public static int Base = 100000;
		public static int BaseDigits = 5;
		public static string BaseFormat = "00000";
		#endregion;
		...
		public MyBigInteger(object value) : this(value.ToString()) { }
		...
		public MyBigInteger(string value)
		{
			...
			for (int i = value.Length - 1; i >= pos; i -= BaseDigits)
			{
				int d = 1;
				for (int j = 0; j < BaseDigits && i - j >= pos; ++j)
				{
					_data[_sz - 1] += d * (value[i - j] - '0');
					d *= 10;
				}
				_sz++;
			}
		...
		public static bool operator <(MyBigInteger a, MyBigInteger b) => a.CompareTo(b) < 0;
		public static bool operator >(MyBigInteger a, MyBigInteger b) => a.CompareTo(b) > 0;
		public static bool operator ==(MyBigInteger a, MyBigInteger b) => a.CompareTo(b) == 0;
		public static bool operator !=(MyBigInteger a, MyBigInteger b) => a.CompareTo(b) != 0;

		public static implicit operator MyBigInteger(int value)
		{
			MyBigInteger res = new MyBigInteger();
			if (value < 0)
			{
			...
			while (value != 0)
			{
				res._data[res._sz++] = value % Base;
				value /= Base;
			}
			res.Norm();
			return res;
		}
		public static implicit operator MyBigInteger(string value)
		{
			if (string.IsNullOrEmpty(value))
				throw new InvalidCastException();
			for (int i = 0; i < value.Length; ++i)
			...
			for (int i = value.Length - 1; i >= pos; i -= BaseDigits)
			{
				...
				for (int j = 0; j < BaseDigits && i - j >= pos; ++j)
				{
					res._data[res._sz - 1] += d * (value[i - j] - '0');
					d *= 10;
				}
				res._sz++;
			}
		...
		/// <summary>
		/// Creates a shallow copy of the MyBigInteger.
		/// </summary>
		/// <returns></returns>
		public object Clone()
		{
