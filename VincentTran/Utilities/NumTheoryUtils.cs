...
namespace VincentTran.Utilities
{
	public static class NumTheoryUtils
	{
		...
		/// </summary>
		public static int Gcd(this int a, int b) => b == 0 ? Math.Abs(a) : b.Gcd(a % b);

		/// <summary>
		/// Computes gcd of two numbers
		/// </summary>
		public static long Gcd(this long a, long b) => b == 0 ? Math.Abs(a) : b.Gcd(a % b);

		/// <summary>
		/// Creates prime number sieve. False means the number is prime.
		/// </summary>
		public static bool[] Sieve(this int n)
		{
			bool[] res = new bool[n];
			if (n > 0) res[0] = true;
			if (n > 1) res[1] = true;
		...
		/// </summary>
		public static bool IsPrimeNaive(this int n)
		{
			if (n < 2) return false;
		...
		private static bool IsCompositeByWitness(this long n, long witness)
		{
			var d = n - 1;
			var s = 0;
			while ((d & 1) == 0)
		...
		/// </summary>
		public static int[] GetMinDivisorArray(this int n)
		{
			var res = new int[n + 1];
		...
		/// The time complexity is O(n*log(n))
		/// </summary>
		public static int[] GetMaxDivisorArray(this int n)
		{
			var res = new int[n + 1];
		...
		/// </summary>
		public static long Inverse(this long a, long n)
		{
		...
		/// </summary>
		public static int Modulo(this int a, int b) => a >= 0 ? a % b : (b - (Math.Abs(a) % b));
		...
		/// a^b mod n
		/// </summary>
		public static long PowWith64Overflow(long a, long b, long mod)
		{
			if (b == 0) return 1;
			else
			{
				long tmp = PowWith64Overflow(a, b / 2, mod);
				tmp = Mul(tmp, tmp, mod);
				if ((b & 1) != 0) tmp = Mul(tmp, a, mod); ;
				return tmp % mod;
			}
		}
		...
		/// </summary>
		public static int EulerPhi(this int n)
		{
			int result = n;
			for (int i = 2; i * i <= n; ++i)
				if (n % i == 0)
		...
		/// Time complexity O(sqrt(N)).
		/// </summary>
		public static long EulerPhi(this long n)
		{
			long result = n;
		...
		/// </summary>
		public static int[] EulerPhiArray(this int n)
		{
			var phi = new int[n + 1];
			for (int i = n; i > 0; i--)
			{
